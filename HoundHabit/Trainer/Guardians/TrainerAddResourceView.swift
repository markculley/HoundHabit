...
struct TrainerAddResourceView: View {
    let guardian: LinkedGuardian
    var onSave: (Resource) -> Void = { _ in }
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Add Resource for \(guardian.profile.fullName ?? "Guardian")")
        ...
        do {
            ...
            if kind == .photo, let data = photoData {
                photoUrl = try await storageService.uploadResourcePhoto(
                    data: data, guardianId: guardian.guardianId, resourceId: resourceId
                )
            }

            let resource = try await service.createResourceForGuardian(
                id: resourceId,
...
#Preview {
    TrainerAddResourceView(guardian: LinkedGuardian(
        id: UUID(),
        trainerId: UUID(),
        guardianId: UUID(),
        status: .active,
